        ...
        <div className="fixed inset-0 z-[100] flex items-start justify-center pt-20 px-4">
          ...
          >
            <div className="p-8 border-b border-slate-100 flex items-center gap-6">
              <Search className="text-brand-accent" size={32} />
              <input
                ref={inputRef}
                type="text"
                value={query}
                ...
                className="flex-grow bg-transparent text-2xl font-black text-brand-primary focus:outline-none placeholder:text-slate-300 uppercase tracking-tighter"
              />
              <button
                onClick={onClose}
                className="p-3 hover:bg-brand-secondary rounded-2xl transition-all text-slate-400"
              >
                <X size={32} />
              </button>
            </div>
              ...
              {query.trim().length < 2 ? (
                ...
                <div className="space-y-12">
                      {results.products.length > 0 && (
                        <div>
                          <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-primary mb-6 flex items-center gap-3">
                            <ShoppingBag size={14} />
                            ...
                            {results.products.map((product) => (
                              ...
                              >
                                ...
                                <div className="flex-grow min-w-0">
                                  <h4 className="text-lg font-black text-brand-primary truncate group-hover:text-brand-accent transition-colors uppercase tracking-tight">
                                    {product.title}
                                  </h4>
                                  <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">
                                    <span className="text-brand-accent">{formatCurrency(product.price)}</span>
                                    <span>•</span>
                                ...
                                <div className="w-12 h-12 rounded-xl bg-brand-secondary flex items-center justify-center text-slate-400 group-hover:bg-brand-accent group-hover:text-white transition-all">
                                  <ArrowRight size={20} />
                                </div>
                              </Link>
                            ))}
                      ...
                      {results.posts.length > 0 && (
                        <div>
                          <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-accent mb-6 flex items-center gap-3">
                            <FileText size={14} />
                          ...
                          <div className="grid grid-cols-1 gap-4">
                            {results.posts.map((post) => (
                              <Link
                                ...
                                className="flex items-center gap-6 p-4 rounded-3xl hover:bg-white hover:shadow-hover transition-all group border border-transparent hover:border-slate-100"
                              >
                                <div className="w-20 h-20 bg-brand-secondary text-brand-accent rounded-2xl flex items-center justify-center shrink-0 shadow-soft">
                                  <FileText size={32} />
                                </div>
                                <div className="flex-grow min-w-0">
                                  <h4 className="text-lg font-black text-brand-primary truncate group-hover:text-brand-accent transition-colors uppercase tracking-tight">
            ...
            <div className="p-6 bg-brand-primary text-center">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-brand-secondary/40">
                Press <kbd className="bg-white/10 px-2 py-1 rounded-lg text-brand-secondary/60 border border-white/10 shadow-sm mx-2">ESC</kbd> to close
              </p>
            </div>
          </motion.div>
        </div>
