...
export default function Categories() {
  return (
    <div className="pt-40 pb-32 bg-slate-50 min-h-screen">
      <Helmet>
        <title>All Categories | AffiliatePro</title>
        <meta name="description" content="Browse our expertly reviewed products by category." />
      </Helmet>
            ...
            <Link key={cat.name} to={`/category/${encodeURIComponent(cat.name)}`} className="group">
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="w-20 h-20 bg-slate-50 text-indigo-600 rounded-3xl flex items-center justify-center mb-10 group-hover:bg-white transition-colors shadow-sm">
                    {cat.icon}
                  </div>
                  <h3 className="text-3xl font-black text-slate-900 mb-4 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{cat.name}</h3>
