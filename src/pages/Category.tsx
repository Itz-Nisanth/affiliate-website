    ...
    <div className="pt-40 bg-slate-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400">
        ...
        {isViewingSubCategory ? (
          <>
            <Link to={`/category/${encodeURIComponent(decodedCategory)}`} className="hover:text-indigo-600 transition-colors">{decodedCategory}</Link>
            <ChevronRight size={12} />
      ...
      <section className="py-24 px-4 bg-white border-b border-slate-100">
        <div className="max-w-7xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-8 shadow-soft border border-indigo-100">
          ...
          <h1 className="text-6xl md:text-8xl font-black text-slate-900 mb-8 tracking-tighter uppercase">
            {isViewingSubCategory ? (
              <>Best <span className="text-indigo-600">{decodedSubCategory}</span></>
            ) : (
              <>Explore <span className="text-indigo-600">{decodedCategory}</span></>
            )}
          </h1>
                ...
                <Link key={sub} to={`/category/${encodeURIComponent(decodedCategory)}/${encodeURIComponent(sub)}`} className="group">
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <h3 className="text-3xl font-black text-slate-900 mb-2 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{sub}</h3>
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                        {PRODUCTS.filter(p => p.category === decodedCategory && p.subCategory === sub).length} Products Available
                      </p>
                    </div>
                    <div className="relative z-10 w-14 h-14 bg-slate-50 text-slate-400 rounded-2xl flex items-center justify-center group-hover:bg-white group-hover:text-indigo-600 transition-all shadow-sm">
                      <ArrowRight size={24} />
                    </div>
                  </motion.div>
                </Link>
            ...
            <>
              {filteredProducts.length > 0 ? (
                ...
                <div className="text-center py-32 bg-white rounded-[3rem] border border-slate-100 shadow-soft">
                  ...
                  <Link to="/categories" className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200">
                    Back to Categories
                  </Link>
                </div>
              )}
      ...
      <section className="py-32 px-4">
        <div className="max-w-5xl mx-auto bg-slate-900 rounded-[4rem] p-12 md:p-24 text-center text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 bg-indigo-600/20 text-indigo-400 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest mb-8 border border-indigo-500/20">
            ...
            <h2 className="text-5xl md:text-7xl font-black mb-8 leading-[0.9] tracking-tighter uppercase">
              Get the <br />
              <span className="text-indigo-500">Best Deals</span>
            </h2>
            ...
            <form className="flex flex-col md:flex-row gap-4 max-w-lg mx-auto">
              <input
                ...
                className="flex-grow bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-medium"
              />
              <button className="bg-white text-slate-900 px-10 py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-50 transition-all shrink-0 active:scale-95">
                Subscribe
              </button>
            </form>
          </div>
