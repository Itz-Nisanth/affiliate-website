...
export default function ProductReview() {
  ...
  return (
    <div className="pt-20 bg-slate-50 min-h-screen">
      <Helmet>
        <title>{product.title} Review (2026) | Expert Analysis & Best Price</title>
        <meta name="description" content={`Read our in-depth ${product.title} review. We analyze performance, specs, and value to help you decide if it's the right choice for you in 2026.`} />
        <meta name="keywords" content={`${product.title} review, ${product.category} reviews, ${product.subCategory} guide, best ${product.subCategory} 2026`} />
        <meta property="og:title" content={`${product.title} Review | AffiliatePro`} />
      ...
      {/* Scroll Progress Bar */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-1.5 bg-brand-accent z-[60] origin-left"
        style={{ scaleX }}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 flex items-center gap-2 text-sm text-slate-500">
        ...
        <ChevronRight size={14} />
        <Link to={`/category/${encodeURIComponent(product.category)}`} className="hover:text-brand-accent transition-colors">{product.category}</Link>
        <ChevronRight size={14} />
        <Link to={`/category/${encodeURIComponent(product.category)}/${encodeURIComponent(product.subCategory)}`} className="hover:text-brand-accent transition-colors">{product.subCategory}</Link>
        <ChevronRight size={14} />
        <span className="text-brand-primary font-medium truncate">{product.title}</span>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <main className="lg:col-span-8">
            ...
            >
              <div className="flex flex-wrap items-center gap-4 mb-6">
                <div className="bg-brand-secondary text-brand-primary px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest">
                   {product.category}
                </div>
              ...
              {/* Summary Box */}
              <div className="bg-brand-primary text-white p-8 rounded-3xl mb-12 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-brand-accent/20 rounded-full blur-3xl -mr-16 -mt-16" />
                ...
                <div className="flex flex-wrap gap-4">
                  <a href={cloakLink(product.amazonUrl)} target="_blank" rel="noopener noreferrer" className="bg-brand-accent text-white px-6 py-3 rounded-xl font-bold hover:bg-white hover:text-brand-primary transition-all flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                <div className="bg-brand-secondary/50 p-8 rounded-3xl border border-brand-primary/10">
                  <h4 className="text-brand-primary font-bold text-lg mb-6 flex items-center gap-2">
                    <Check className="text-brand-primary" /> Pros
                  </h4>
                  <ul className="space-y-4">
                    {product.pros.map((pro, i) => (
                      <li key={i} className="flex gap-3 text-slate-800 text-sm font-medium">
                        <div className="w-5 h-5 bg-brand-primary/20 rounded-full flex items-center justify-center shrink-0 mt-0.5">
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 list-none p-0">
                  {product.features.map((feature, i) => (
                    <li key={i} className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-start gap-3">
                      <Zap size={18} className="text-brand-primary shrink-0 mt-1" />
          ...
          <aside className="lg:col-span-4">
            <div className="sticky top-24 space-y-8">
              ...
              <div className="bg-white rounded-[2rem] p-8 shadow-xl border border-slate-100 ring-1 ring-brand-accent/10">
                ...
                <div className="text-center mb-6">
                  <div className="text-3xl font-black text-brand-primary mb-1">{formatCurrency(product.price)}</div>
                  <div className="flex justify-center gap-1 mb-2">
                    {[...Array(5)].map((_, i) => <Star key={i} size={16} className="fill-brand-accent text-brand-accent" />)}
                  </div>
                  <div className="text-slate-400 text-xs font-bold uppercase tracking-widest">Highly Rated on Amazon</div>
                </div>
                <div className="space-y-3">
                  <a href={cloakLink(product.amazonUrl)} target="_blank" rel="noopener noreferrer" className="w-full bg-brand-primary text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-brand-accent transition-all shadow-lg shadow-brand-primary/20 active:scale-95">
                    <ShoppingCart size={20} />
                    Buy on Amazon
                  </a>
                  <button className="w-full bg-brand-secondary text-brand-primary py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-brand-accent hover:text-white transition-all">
                    <Share2 size={18} />
                    Share Review
                  </button>
                </div>
              ...
              <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                ...
                <nav className="space-y-4">
                  {['Summary', 'Pros & Cons', 'Performance', 'Specifications', 'FAQ'].map((item) => (
                    <a key={item} href={`#${item.toLowerCase()}`} className="block text-sm font-medium text-slate-500 hover:text-brand-primary transition-colors">
                      {item}
                    </a>
                  ))}
                </nav>
